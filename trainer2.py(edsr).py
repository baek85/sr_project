...
class Trainer(object):
    ...
    def train(self):
        ...
        for batch_num, (LR, HR, filename) in enumerate(self.training_loader):
            ...
            if(batch_num+1) % self.args.print_every == 0:
                print_save("    Average Loss: {:.4f}    overall time: {:.4f} + {:.4f}"
                .format((train_loss / train_length), self.data_timer.overall, self.train_timer.overall), self.text_path)
                self.data_timer.start()
                self.train_timer.start()
            else:
                self.data_timer.go()
        ...
        print_save("    overall time: {:.4f}".format(self.data_timer.overall), self.text_path)
    ...
    def test(self):
        ...
        avg_msssim = 0
        with torch.no_grad():
            for idx_data, d in enumerate(self.testing_loader):
